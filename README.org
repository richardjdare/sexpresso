...
*note from richardjdare:* I have hacked around with this fork for my own purposes, mainly to handle Common Lisp source code. *It may not do what you want!* Please take a look at the original [(https://github.com/BitPuffin/sexpresso)]
 ...
 * Sexpresso now understands Common Lisp types: SYMBOL, STRING, CHAR, HEX, OCTAL, BIN, PATHNAME. Sexps can have a sub-type: VECTOR or COMPLEX.
 * Sexpresso can now handle Common Lisp quoting syntax and macro characters. (QUOTE, BACKQUOTE, FUNCQUOTE, COMMASPLICE, ATSPLICE, DOTSPLICE) They are stored, in order of appearance, in a std::vector, "attributes", belonging to each Sexp node.
 * The parser can handle Common Lisp single line, and nested block comments.
 * The parser can handle broken input and will return the data it could successfully parse. When it hits a parse error, it closes off the sexps it successfully parsed and returns them. An identifying symbol is added at the point of error.
 * I've ported the existing unit tests to use Qt's unit testing framework, and added a bunch of tests for the new functionality.
